...
def binary_cross_entropy(pred, gt, eps=1e-6):
    bce = -(gt * torch.log(pred + eps) + (1 - gt) * torch.log(1 - pred + eps)).mean()
    return bce
...
def cal_gradient(data):
    ...
    grad_x = F.conv2d(data, kernel_x, padding='same')
    grad_y = F.conv2d(data, kernel_y, padding='same')
...
def bilateral_smooth_loss(data, image, mask):
    ...
    smooth_loss = (data_grad * torch.exp(-rgb_grad.detach()) * mask).mean()

    return smooth_loss

def base_smooth_loss(data, mask):
    """
    image: [C, H, W]
    data: [C, H, W]
    mask: [C, H, W]
...
def loss_cls_3d(features, predictions, k=5, max_points=200000, sample_size=1000, eps=1e-6):
    """
    ...
    sample_preds_pos = sample_preds  # P(class=1)
    sample_preds_neg = 1 - sample_preds  # P(class=0)
    
    neighbor_preds_pos = neighbor_preds  # P(class=1) for neighbors
    neighbor_preds_neg = 1 - neighbor_preds  # P(class=0) for neighbors

    # Stack to create proper probability distributions
    sample_preds_dist = torch.cat([sample_preds_neg, sample_preds_pos], dim=-1)  # shape: (sample_size, 2)
    neighbor_preds_dist = torch.cat([neighbor_preds_neg, neighbor_preds_pos], dim=-1)  # shape: (sample_size, k, 2)

    # Compute KL divergence
    kl = sample_preds_dist.unsqueeze(1) * (torch.log(sample_preds_dist.unsqueeze(1) + eps) - 
                                          torch.log(neighbor_preds_dist + eps))

    loss = kl.sum(dim=-1).mean()

    # Normalize loss - for binary classification, num_classes = 2
    normalized_loss = loss / 2
