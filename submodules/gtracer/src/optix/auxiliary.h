...
// 最大堆向下调整
__forceinline__ __host__ __device__ void heapifyDown(HitInfo *heap, int idx, int size) {
    while (true) {
        int left = 2 * idx + 1;
        int right = 2 * idx + 2;
        int largest = idx;
        if (left < size && heap[left].t > heap[largest].t) largest = left;
        if (right < size && heap[right].t > heap[largest].t) largest = right;
        if (largest != idx) {
            host_device_swap<HitInfo>(heap[idx], heap[largest]);
            idx = largest;
        } else break;
    }
}
...
// 插入hit到最大堆
__forceinline__ __host__ __device__ void insertHitMaxHeap(HitInfo* heap, HitInfo newHit) {
    if (newHit.t < heap[0].t) {
		heap[0] = newHit;
		heapifyDown(heap, 0, MAX_BUFFER_SIZE);
	}
}

__forceinline__ __host__ __device__ HitInfo PopMaxHeap(HitInfo* heap, int &count) {
	if (count <= 0) return HitInfo{1e16f, -1}; // 堆空
	HitInfo top = heap[0];
	heap[0] = heap[count - 1];
	count--;
	heapifyDown(heap, 0, count);
	return top;
}
