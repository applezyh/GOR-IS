...
def create_videos(base_dir, input_dir, out_name, num_frames=480):
  ...
  for k in ['depth', 'normal', 'depth_normal', 'color', 'fresnel', 'roughness', 'diffuse', 'specular']:
    video_file = os.path.join(base_dir, f'{video_prefix}_{k}.mp4')
    input_format = 'gray' if k == 'alpha' or k == 'roughness' else 'rgb'
    

    file_ext = 'tiff' if k in ['depth'] else 'png'
    idx = 0

    if k == 'color':
      file0 = os.path.join(input_dir, 'renders', f'{idx_to_str(0)}.{file_ext}')
    elif k == 'depth' or k == 'normal' or k == 'depth_normal':
      file0 = os.path.join(input_dir, 'vis', f'{k}_{idx_to_str(0)}.{file_ext}')
    else:
       file0 = os.path.join(input_dir, 'pbr', f'{k}_{idx_to_str(0)}.{file_ext}')
    ...
    with media.VideoWriter(
      ...
      for idx in tqdm(range(num_frames)):
        ...
        if k == 'color':
          ...
          img_file = os.path.join(input_dir, 'pbr', f'{k}_{idx_to_str(idx)}.{file_ext}')

        if not os.path.exists(img_file):
          ValueError(f'Image file {img_file} does not exist.')
        img = load_img(img_file)
        if k in ['color', 'normal', 'depth_normal', 'alpha', 'fresnel', 'roughness', 'diffuse', 'specular']:
          img = img / 255.
...
def weighted_blur(
    ...
    """
    blur with normal-based weight correction.
    Args:
        image: [C,H,W]
        normal: [3,H,W] (normalized vectors)
        kernel_size: int
    Returns:
        blurred: [C,H,W]
    """
    ...
    p = kernel_size // 2
    # unfold image
    img = F.unfold(image.unsqueeze(0), kernel_size=(k, k), padding=(p, p))
    img = img.view(1, C, k * k, H*W).squeeze(0)  # [3, k*k, H*W]

    # unfold normal
    norm = F.unfold(normal.unsqueeze(0), kernel_size=(k, k), padding=(p, p))
    norm = norm.view(3, k * k, H*W) # [3, k*k, H*W]
    center_norm = normal.view(3, H*W)  # [3, H*W]
    dot = (norm * center_norm.unsqueeze(1)).sum(dim=0).clamp(0, 1) # [k*k, H*W]
    norm_weights = dot.pow(2)  # [k*k, H*W]

    # normalize weights
    kernel = norm_weights.reshape(-1, H * W)
    kernel = kernel / (kernel.sum(dim=0,keepdim=True) + 1e-6)  # [k*k, H*W]

    out = (img * kernel.unsqueeze(0)).sum(dim=1) # [3, H*W]

    return out.reshape(-1, H, W)


def screen_mips(image, normal, roughness, kernel_size=5, mip_level=4):
    mips = [image]
    H, W = image.shape[-2:]
    level = 1
    while level <= mip_level:
        img = weighted_blur(mips[-1], normal, kernel_size)
        img = F.interpolate(img[None], scale_factor=0.5, mode="bilinear", align_corners=False)[0]
        normal = F.interpolate(normal[None], scale_factor=0.5, mode="bilinear", align_corners=False)[0]
        mips.append(img)
        level += 1

    for idx in range(1, len(mips)):
        mips[idx] = F.interpolate(mips[idx][None], (H, W), mode="bilinear", align_corners=False)[0]

    mips = torch.stack(mips, dim=0) # [M, C, H, W]
    C, H, W = mips.shape[-3:]
    _H, _W = roughness.shape[-2:]
    assert H == _H and W == _W
    uv = torch.cat((2 * roughness - 1, torch.zeros_like(roughness)), dim=0).permute(1, 2, 0).reshape(H * W, 1, 1, -1) # [HW, 1, 1, 2]
    mips = mips.permute(2, 3, 1, 0).reshape(H * W, C, 1, -1) # [HW, 3, 1, M]
    out = torch.nn.functional.grid_sample(mips, uv, mode="bilinear", align_corners=False, padding_mode="border").reshape(H, W, C).permute(2, 0, 1)
